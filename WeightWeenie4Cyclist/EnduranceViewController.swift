...
class EnduranceViewController: UIViewController {
    ...
    func updateTime() {
        ...
        if seconds >= 40 { // stop timer after 10 seconds.
            setCount += 1
            setLabel.text = "\(setCount)"
            inLabel.text = ""
    ...
    func prepareRestingTimer() {
        ...
        if (!timer.isValid) {
            inLabel.text = ""
            let rSelector : Selector = #selector(EnduranceViewController.restingTimer)
    ...
    @IBAction func instructionButton(_ sender: AnyObject) {
        ...
        let ac = UIAlertController(title: "Endurance and Power!", message: "In the Medium gear push hard for 40 seconds. then recover for 20 seconds Repeat 10 times.", preferredStyle: .actionSheet)
