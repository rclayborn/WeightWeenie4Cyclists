...
class ClimbViewController: UIViewController {
    ...
    @IBAction func start(_ sender: AnyObject) {
        if (!timer.isValid) {
            ...
            let aSelector : Selector = #selector(ClimbViewController.updateTime)
    ...
    @objc func updateTime() {
        ...
        if seconds >= 30 { // stop timer after 10 seconds.
            setCount += 1
            setLabel.text = "\(setCount)"
            inLabel.text = ""
    ...
    @objc func prepareRestingTimer() {
        ...
        if (!timer.isValid) {
            inLabel.text = ""
            let rSelector : Selector = #selector(ClimbViewController.restingTimer)
    ...
    @objc func restingTimer() {
        inLabel.text = "RESTING: Cool Down"
        let currentTime = Date.timeIntervalSinceReferenceDate
    ...
    @IBAction func instructionButton(_ sender: AnyObject) {
        ...
        let ac = UIAlertController(title: "Climb Stronger!", message: "On a moderate incline, stand out of the saddle and go as fast as you can for 30 seconds. cool down and repeat sitted. Do six sets", preferredStyle: .actionSheet)
